...
namespace SAT
{
    public class Bullet : MonoBehaviour
    {
        ...
        public static bool naveDestruida = false;
        public static bool jefeDestruido = false;
        ...
        private void MoveBullet()
        {
            ...
            {
                // La bala ha golpeado
                // Si la bala golpeó un objeto con Script Enemigo, reducir su vida y destruir a 0.
                if (rayHit.transform.gameObject.GetComponent<Enemigo>() != null){
                    Enemigo ene = rayHit.transform.gameObject.GetComponent<Enemigo>();
                    ene.vida -= 1;
                    if (ene.vida <= 0){
                        Destroy(rayHit.transform.gameObject);
                    }
                // Si la bala golpeó un objeto con Script EnemigoF, reducir su vida y destruir a 0.
                // cambiar variable de destruido a true.
                }else  if (rayHit.transform.gameObject.GetComponent<EnemigoF>() != null){
                    EnemigoF enef = rayHit.transform.gameObject.GetComponent<EnemigoF>();
                    enef.vida -= 1;
                    if (enef.vida <= 0){
                        Destroy(rayHit.transform.gameObject);
                        jefeDestruido = true;
                    ...
                    // Si la bala golpeó un objeto con tag Asteroide, se destruye.  
                    }else if (rayHit.transform.gameObject.CompareTag("Asteroide")){
                        Destroy(rayHit.transform.gameObject);

                // Si la bala golpeó al jugador, reducir vida y destruir a 0.
                // cambiar variable de destruido a true.
                    }else if (rayHit.transform.gameObject.GetComponent<Ship>() != null){
                    Ship player = rayHit.transform.gameObject.GetComponent<Ship>();
                    player.vida -= 1;
                    if (player.vida <= 0){
                        Destroy(rayHit.transform.gameObject);
                        naveDestruida = true;
